...
const MedicalNotes = ({ patientId }) => {
  ...
  const { data: notesData, isLoading: isLoadingNotes, error: errorNotes } = useNotes(patientId);
  ...
  const handleSaveNote = () => {
    ...
    if (editingNoteId !== null) {
      ...
      updateNoteMutation.mutate({ noteId: editingNoteId, patientId: patientId, ...notePayload });
    } else {
      // Add a new note
  ...
  const getCategoryInfo = (categoryId) => {
    return noteCategories.find((cat) => cat.id === categoryId) || noteCategories[3];
  };
  ...
  const Note = ({ note }) => {
    ...
    const isDeleting = deleteNoteMutation.isLoading && deleteNoteMutation.variables?.noteId === note.id;

    return (
      <div
        className={`bg-white p-4 rounded-lg shadow mb-4 border-l-4 ${isDeleting ? 'opacity-50' : ''}`}
        style={{ /* ... border color style ... */
            borderLeftColor: category.color.startsWith('bg-red') ? '#fecaca' :
                             category.color.startsWith('bg-blue') ? '#bfdbfe' :
                             category.color.startsWith('bg-green') ? '#bbf7d0' :
                             '#e5e7eb',
        }}
      >
        <div className="flex justify-between items-start">
          ...
          <div className="flex space-x-2">
            <button onClick={() => handleEditNote(note)} className="text-indigo-600 hover:text-indigo-900 disabled:opacity-50" title="Edit Note" disabled={isDeleting}>
              <Edit className="h-4 w-4" />
            </button>
            <button onClick={() => handleDeleteNote(note.id)} className="text-red-600 hover:text-red-900 disabled:opacity-50" title="Delete Note" disabled={isDeleting}>
  ...
  return (
    <div className="bg-gray-50 p-4 rounded-lg">
      <div className="flex justify-between items-center mb-6">
        ...
        >
          {showAddNote ? ( <><Clock className="h-4 w-4 mr-2" />Cancel</> ) : ( <><Plus className="h-4 w-4 mr-2" />Add Note</> )}
        </button>
      </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6 border border-gray-200">
          {/* ... Form inputs remain the same ... */}
           <div className="mb-4">
             ...
             <input type="text" name="title" className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value={newNote.title} onChange={handleNoteInputChange} placeholder="Brief title for the note"/>
           </div>
           <div className="mb-4">
             <label className="block text-sm font-medium text-gray-700 mb-1">Note Category</label>
             <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
               {noteCategories.map((category) => (
                 <div key={category.id} className={`flex items-center p-2 rounded cursor-pointer border ${ newNote.category === category.id ? 'ring-2 ring-indigo-500 border-indigo-500' : 'border-gray-200 hover:bg-gray-50' }`} onClick={() => setNewNote({ ...newNote, category: category.id })}>
                   <div className={`p-1 rounded-full ${category.color.split(' ')[0]}`}><category.icon className={`h-4 w-4 ${category.color.split(' ')[1]}`} /></div>
           ...
           <div className="mb-4">
             ...
             <textarea name="content" rows="4" className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value={newNote.content} onChange={handleNoteInputChange} placeholder="Detailed medical notes, observations, or instructions..."></textarea>
           </div>
          <div className="flex justify-end">
            <button
              ...
              disabled={!newNote.title.trim() || !newNote.content.trim() || isSaving}
            >
      ...
      {/* Notes List */}
      {notes.length > 0 ? (
        notes.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((note) => <Note key={note.id} note={note} />)
      ) : (
        <div className="bg-white p-6 rounded-lg shadow text-center">
          <FileText className="h-12 w-12 mx-auto text-gray-400 mb-2" />
          <h3 className="text-lg font-medium text-gray-900">No medical notes</h3>
          <p className="mt-1 text-sm text-gray-500">No medical notes have been added for this patient yet.</p>
        </div>
      )}
    </div>
  );
