...
const DosesFormSection = ({ doses = [], onChange }) => {
  ...
  const handleDoseChange = (e, field, doseId) => {
    ...
    if (doseId) {
      onChange(doses.map((dose) => dose.id === doseId ? { ...dose, [field]: value } : dose));
    } else {
...
const ProductManagement = () => {
  ...
  // --- Mutation Hooks ---
  const addProductMutation = useCreateProduct({ onSuccess: () => setShowProductModal(false) }); // Renamed from useAddProduct
  const updateProductMutation = useUpdateProduct({ onSuccess: () => setShowProductModal(false) });
  const deleteProductMutation = useDeleteProduct();
  const addPlanMutation = useAddSubscriptionPlan({ onSuccess: () => setShowPlanModal(false) });
  const updatePlanMutation = useUpdateSubscriptionPlan({ onSuccess: () => setShowPlanModal(false) });
  ...
  // --- Derived Data ---
  const productCategories = [...new Set([...allProducts.map(p => p.category), ...allSubscriptionPlans.map(p => p.category)])].filter(Boolean).sort();
  ...
  const filteredPlans = allSubscriptionPlans.filter(p =>
    (categoryFilter === 'all' || p.category === categoryFilter) &&
    ((p.name || '').toLowerCase().includes(searchTerm.toLowerCase()) || (p.description || '').toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  const handleEditProduct = (product) => {
    ...
    setProductFormData({
      ...
      doses: Array.isArray(product.doses) ? product.doses.map(d => ({...d, id: d.id || `temp_${Date.now()}`})) : [], // Ensure doses have IDs
  ...
  const handleDeleteProduct = (id) => {
    ...
    const isUsed = allSubscriptionPlans.some(plan =>
        Array.isArray(plan.allowedProductDoses) &&
        plan.allowedProductDoses.some(doseRef => doseRef.productId === id)
    );
    if (isUsed) {
      toast.error('This product cannot be deleted as one of its doses is used in a subscription plan.');
      return;
    }
  ...
  // Utility functions
  const getServiceNameById = (id) => services.find(s => s.id === id)?.name || null;
  const formatCategoryName = (category) => category ? category.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : 'Uncategorized';
  const formatFulfillmentSource = (source) => source ? source.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'N/A';
  ...
  const isMutating = addProductMutation.isLoading || updateProductMutation.isLoading || deleteProductMutation.isLoading || addPlanMutation.isLoading || updatePlanMutation.isLoading || deletePlanMutation.isLoading;
  const error = errorServices || errorProducts || errorPlans;

  if (isLoading) {
    return <div className="flex justify-center items-center p-8 h-64"><Loader2 className="h-8 w-8 animate-spin text-indigo-600" /></div>;
  }

  if (error) {
     return <div className="p-8 text-center text-red-600"><AlertTriangle className="h-10 w-10 mx-auto mb-2" /><p>Error loading data.</p></div>;
  }
    ...
    <div>
      ...
      <SearchAndFilters
        ...
        filters={[
          ...
          ...(activeTab === 'products' && Array.isArray(services) && services.length > 0 ? [{ value: serviceFilter, onChange: setServiceFilter, options: [{ value: 'all', label: 'All Services' }, ...services.map(s => ({ value: s.id.toString(), label: s.name }))], icon: Briefcase }] : []),
      ...
      {activeTab === 'products' && (
        <>
          ...
          <div className="bg-white shadow overflow-hidden rounded-lg mt-6"> {/* Added margin top */}
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredProducts.flatMap(product => {
                    if (product.type !== 'medication' || !Array.isArray(product.doses) || product.doses.length === 0) {
                    ...
                    <tr key={product.key} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4"><div className="flex flex-wrap gap-1 max-w-xs">{Array.isArray(product.associatedServiceIds) && product.associatedServiceIds.map(serviceId => { const serviceName = getServiceNameById(serviceId); return serviceName ? <Badge key={serviceId} className="bg-gray-100 text-gray-800">{serviceName}</Badge> : null;})}{(!Array.isArray(product.associatedServiceIds) || product.associatedServiceIds.length === 0) && <span className="text-xs text-gray-500">None</span>}</div></td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatFulfillmentSource(product.fulfillmentSource)}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{product.allowOneTimePurchase ? <span className="text-xs text-green-600 flex items-center"><Check size={12} className="mr-1" />Allowed (${Number(product.price).toFixed(2)})</span> : <span className="text-xs text-gray-500">Subscription Only</span>}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">{(!product.currentDose || product.isFirstDose) && (<><button className="text-indigo-600 hover:text-indigo-900 mr-3 disabled:opacity-50" onClick={() => handleEditProduct(allProducts.find(p => p.id === product.id))} disabled={isMutating}><Edit className="h-5 w-5" /></button><button className="text-red-600 hover:text-red-900 disabled:opacity-50" onClick={() => handleDeleteProduct(product.id)} disabled={isMutating || deleteProductMutation.variables === product.id}>{deleteProductMutation.isLoading && deleteProductMutation.variables === product.id ? <Loader2 className="h-5 w-5 animate-spin" /> : <Trash2 className="h-5 w-5" />}</button></>)}</td>
                    </tr>
                ))}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Included Doses</th>
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredPlans.map(plan => (
                  <tr key={plan.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"><button className="text-indigo-600 hover:text-indigo-900 mr-3 disabled:opacity-50" onClick={() => handleEditPlan(plan)} disabled={isMutating}><Edit className="h-5 w-5" /></button><button className="text-red-600 hover:text-red-900 disabled:opacity-50" onClick={() => handleDeletePlan(plan.id)} disabled={isMutating || deletePlanMutation.variables === plan.id}>{deletePlanMutation.isLoading && deletePlanMutation.variables === plan.id ? <Loader2 className="h-5 w-5 animate-spin" /> : <Trash2 className="h-5 w-5" />}</button></td>
                  </tr>
                ))}
      ...
      {/* Product Modal */}
      <Modal title={editMode ? 'Edit Product' : 'Add New Product'} isOpen={showProductModal} onClose={() => setShowProductModal(false)} onSubmit={handleProductSubmit} submitText={editMode ? 'Save Changes' : 'Add Product'} isSubmitting={addProductMutation.isLoading || updateProductMutation.isLoading}>
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-5">
             ...
             <FormSelect label="Fulfillment Source" name="fulfillmentSource" value={productFormData.fulfillmentSource} onChange={handleProductInputChange} options={[{ value: 'compounding_pharmacy', label: 'Compounding Pharmacy' }, { value: 'retail_pharmacy', label: 'Retail Pharmacy' }, { value: 'internal_supplement', label: 'Internal (Supplement)' }, { value: 'internal_service', label: 'Internal (Service)' }]}/>
             ...
             {productFormData.type !== 'medication' && <FormInput label="Price (One-Time)" name="price" type="number" min="0" step="0.01" value={productFormData.price} onChange={handleProductInputChange} prefix="$" />}
             {productFormData.type === 'medication' && <FormInput label="One-Time Purchase Price" name="oneTimePurchasePrice" type="number" min="0" step="0.01" value={productFormData.oneTimePurchasePrice} onChange={handleProductInputChange} prefix="$" />}
          </div>
          {/* Right Column */}
          <div className="col-span-7">
             ...
             <div className={productFormData.type === 'medication' ? 'mt-4' : ''}>
               ...
               {/* Associated Services */}
               <div className="mb-4 mt-4"><label className="block text-sm font-medium text-gray-700 mb-1">Associated Services (Standalone)</label><div className="border border-gray-300 rounded-md p-2 h-32 overflow-y-auto space-y-1">{Array.isArray(services) ? services.map(service => (<div key={service.id} className="flex items-center"><input type="checkbox" id={`service-assoc-${service.id}`} checked={productFormData.associatedServiceIds?.includes(service.id) || false} onChange={() => handleServiceSelectionChange(service.id)} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"/><label htmlFor={`service-assoc-${service.id}`} className="ml-2 text-sm text-gray-700">{service.name}</label></div>)) : <p className="text-xs text-gray-500">Loading services...</p>}</div></div>
               <div className="flex space-x-6 mt-4">
                 ...
                 {productFormData.type !== 'medication' && <FormCheckbox label="Allow One-Time Purchase" name="allowOneTimePurchase" checked={!!productFormData.allowOneTimePurchase} onChange={handleProductInputChange}/>}
               </div>
             </div>
          </div>
        </div>
      ...
      {/* Plan Modal */}
      <Modal title={editMode ? 'Edit Plan' : 'Add New Plan'} isOpen={showPlanModal} onClose={() => setShowPlanModal(false)} onSubmit={handlePlanSubmit} submitText={editMode ? 'Save Changes' : 'Add Plan'} isSubmitting={addPlanMutation.isLoading || updatePlanMutation.isLoading}>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div> {/* Left Column */}
             ...
             <FormInput label="Price" name="price" type="number" min="0" step="0.01" value={planFormData.price} onChange={handlePlanInputChange} prefix="$" />
             <div className="grid grid-cols-2 gap-4">
           ...
           <div> {/* Right Column */}
             <div className="mb-4"><label className="block text-sm font-medium text-gray-700 mb-1">Included Product Doses</label><div className="border border-gray-300 rounded-md p-2 h-64 overflow-y-auto space-y-2">{allProducts.filter(p => p.type === 'medication' && p.doses?.length > 0).map(product => (<div key={product.id}><h4 className="text-xs font-semibold text-gray-600 mb-1">{product.name}</h4><div className="pl-2 space-y-1">{product.doses.map(dose => (<div key={dose.id} className="flex items-center"><input type="checkbox" id={`dose-select-${product.id}-${dose.id}`} checked={planFormData.allowedProductDoses?.some(d => d.productId === product.id && d.doseId === dose.id) || false} onChange={() => handleDoseSelectionChange(product.id, dose.id)} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"/><label htmlFor={`dose-select-${product.id}-${dose.id}`} className="ml-2 text-sm text-gray-700">{dose.value} {dose.description ? `(${dose.description})` : ''}</label></div>))}</div></div>))}{allProducts.filter(p => p.type === 'medication' && p.doses?.length > 0).length === 0 && <p className="text-xs text-gray-500">No medication products with doses available.</p>}</div><p className="mt-1 text-xs text-gray-500">{planFormData.allowedProductDoses?.length || 0} doses selected</p></div>
             <div className="grid grid-cols-2 gap-4">
