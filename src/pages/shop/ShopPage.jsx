...
import { Package, Search, Filter, PlusCircle, CheckCircle, FileText } from 'lucide-react';
...
const mockProducts = [
  {
    ...
    doses: [ // Price removed from doses
      { id: 101, value: '0.25mg', description: 'Starting dose', allowOneTimePurchase: true }, // Trial dose allowed one-time
      { id: 102, value: '0.5mg', description: 'Standard dose', allowOneTimePurchase: false },
      { id: 103, value: '1.0mg', description: 'Higher dose', allowOneTimePurchase: false }
    ]
  },
  {
    id: 2,
    ...
    doses: [ // Price removed from doses
      { id: 201, value: '5mg', description: 'Standard dose', allowOneTimePurchase: false },
      { id: 202, value: '10mg', description: 'Higher dose', allowOneTimePurchase: false }
    ]
  },
  {
    id: 3,
...
const ProductCard = ({ product, onAddToCart }) => {
  const [selectedDoseId, setSelectedDoseId] = useState(
    product.type === 'medication' && product.doses?.length > 0 ? product.doses[0].id : null
  );
  ...
  const selectedDose = product.type === 'medication'
  ...
  const handleAddToCartClick = () => {
    ...
    if (!isOneTimePurchaseAllowed()) {
       message.warning(`"${product.name}${selectedDose ? ` (${selectedDose.value})` : ''}" is only available via subscription.`);
       return;
    }
    ...
    if (product.type === 'medication') {
      ...
      if (product.price !== undefined) {
         ...
         const pseudoDoseData = { id: `product-${product.id}`, value: 'Standard', price: product.price };
  ...
  const currentFeedbackId = product.type === 'medication' ? selectedDoseId : `product-${product.id}`;
      ...
      <div className="p-4 flex-grow flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-1">
            ...
            {product.requiresPrescription && (
              <span title="Requires Prescription" className="ml-2 text-blue-600 flex items-center text-xs bg-blue-100 px-1.5 py-0.5 rounded">
        ...
        <div className="mt-4">
          {product.type === 'medication' && product.doses?.length > 0 ? (
            <div className="mb-3">
              <label htmlFor={`dose-${product.id}`} className="block text-sm font-medium text-gray-700 mb-1">
                Select Dose:
              </label>
              <select
                id={`dose-${product.id}`}
              ...
              >
                ...
                {product.doses.map(dose => (
                  <option key={dose.id} value={dose.id}>
                    {dose.value} {dose.allowOneTimePurchase ? '' : '(Subscription Only)'}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <p className="text-xl font-bold text-gray-900">
              {displayPrice !== undefined ? `$${displayPrice.toFixed(2)}` : 'N/A'}
              {product.type === 'medication' && <span className="text-sm font-normal text-gray-500"> / one-time</span>}
            </p>
          </div>
          ...
          <button
            ...
            title={
              ...
              !isOneTimePurchaseAllowed() ? "This option is subscription only." :
              ""
            }
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      const updatedMockProducts = mockProducts.map(p => ({
          ...p,
          allowOneTimePurchase: p.allowOneTimePurchase !== undefined ? p.allowOneTimePurchase : (p.type !== 'medication'), // Default non-meds to true if undefined
          oneTimePurchasePrice: p.oneTimePurchasePrice !== undefined ? p.oneTimePurchasePrice : p.price, // Use price if oneTime is missing for meds
          doses: p.doses?.map(d => ({
              ...d,
              allowOneTimePurchase: d.allowOneTimePurchase !== undefined ? d.allowOneTimePurchase : false // Default doses to false if undefined
          })) || []
      }));
