...
-- Fix the patients and consultations tables
DO $$
BEGIN
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'patients') THEN
        ...
        IF EXISTS (SELECT FROM information_schema.columns 
            ...
            ALTER TABLE patients ADD COLUMN temp_id UUID DEFAULT uuid_generate_v4();
            ...
            IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'consultations') THEN
                ...
                -- Update the temporary column with UUID values that match patients
            ...
            -- Now we can safely modify the patients table
            -- Drop the old id column and rename the new one
            ALTER TABLE patients DROP COLUMN id CASCADE;
            ALTER TABLE patients RENAME COLUMN temp_id TO id;
