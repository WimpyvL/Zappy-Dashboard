...
BEGIN
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'client_record') THEN
        ...
        IF NOT EXISTS (SELECT FROM information_schema.columns 
            ...
            ALTER TABLE client_record ADD COLUMN date_created TIMESTAMP WITH TIME ZONE DEFAULT NOW();
        END IF;
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'order') THEN
        -- Check if client_record_id column exists and is a foreign key
        IF EXISTS (SELECT FROM information_schema.columns 
                  WHERE table_schema = 'public' 
                  AND table_name = 'order' 
                  AND column_name = 'client_record_id') THEN
            -- Drop the foreign key constraint if it exists
            -- We'll handle the relationship in the application code instead
            ALTER TABLE "order" DROP CONSTRAINT IF EXISTS fk_order_client_record;
        END IF;
    END IF;
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'session') THEN
        ...
        IF EXISTS (SELECT FROM information_schema.columns 
            ...
            -- Drop the foreign key constraint if it exists
            -- We'll handle the relationship in the application code instead
            ALTER TABLE session DROP CONSTRAINT IF EXISTS fk_session_client_record;
        END IF;
    END IF;
END $$;
